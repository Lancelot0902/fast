...
def _train_loader_from_config(cfg, *, train_set=None, transforms=None, sampler=None, **kwargs):
    ...
    if train_set is None:
        ...
        for d in cfg.DATASETS.NAMES:
            ...
            if comm.is_main_process():
                data.show_train()
            train_items.extend(data.train)  # (img_path, pid, camid)

        train_set = CommDataset(train_items, transforms, relabel=True)  # (img, pid, camid, img_path)
    ...
    if sampler is None:
        ...
        elif sampler_name == "DomainSingleNaiveIdentitySampler":
            sampler = samplers.DomainSingleNaiveIdentitySampler(train_set.img_items, mini_batch_size, num_instance)
        elif sampler_name == "DomainMultiNaiveIdentitySampler":
            sampler = samplers.DomainMultiNaiveIdentitySampler(train_set.img_items, mini_batch_size, num_instance)
        else:
