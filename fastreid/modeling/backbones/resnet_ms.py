...
from ...layers.mixstyle import MixStyle
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, with_in, mixstyle_layers, mixstyle_p, mixstyle_alpha, block, layers, non_layers):
        self.inplanes = 64
        super().__init__()
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], 1, bn_norm, with_ibn, with_se, with_in)
        self.layer2 = self._make_layer(block, 128, layers[1], 2, bn_norm, with_ibn, with_se, with_in)
        self.layer3 = self._make_layer(block, 256, layers[2], 2, bn_norm, with_ibn, with_se, with_in)
    ...
    def forward(self, x,):
        ...
        if len(self.NL_2_idx) == 0:
            self.NL_2_idx = [-1]
        for i in range(len(self.layer2)):
            x_ms = self.layer2[i](x_ms)
            if i == self.NL_2_idx[NL2_counter]:
                _, C, H, W = x_ms.shape
                x_ms = self.NL_2[NL2_counter](x_ms)
                NL2_counter += 1

        if 'layer2' in self.mixstyle_layers:
            x_ms = self.mixstyle(x_ms)
        ...
        if len(self.NL_3_idx) == 0:
            self.NL_3_idx = [-1]
        for i in range(len(self.layer3)):
            x_ms = self.layer3[i](x_ms)
            if i == self.NL_3_idx[NL3_counter]:
                _, C, H, W = x_ms.shape
                x_ms = self.NL_3[NL3_counter](x_ms)
                NL3_counter += 1
        ...
        if len(self.NL_4_idx) == 0:
            self.NL_4_idx = [-1]
        for i in range(len(self.layer4)):
            x_ms = self.layer4[i](x_ms)
            if i == self.NL_4_idx[NL4_counter]:
                _, C, H, W = x_ms.shape
                x_ms = self.NL_4[NL4_counter](x_ms)
                NL4_counter += 1
...
def build_resnet_ms_backbone(cfg):
    """
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_se, with_nl, with_in, mixstyle_layers, mixstyle_p, mixstyle_alpha, block,
