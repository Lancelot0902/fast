...
from ...layers.fliter import GlobalFilter
...
class ResNet(nn.Module):
    def __init__(self, last_stride, bn_norm, with_ibn, with_se, with_nl, with_in, mixstyle_layers, block, layers, non_layers):
        self.inplanes = 64
        super().__init__()
        ...
        self.layer1 = self._make_layer(block, 64, layers[0], 1, bn_norm, with_ibn, with_se, with_in)    # [64, 256, 56, 56]
        self.layer2 = self._make_layer(block, 128, layers[1], 2, bn_norm, with_ibn, with_se, with_in)   # [64, 512, 28, 28]
        self.layer3 = self._make_layer(block, 256, layers[2], 2, bn_norm, with_ibn, with_se, with_in)   # [64, 1024, 14, 14]
        self.layer4 = self._make_layer(block, 512, layers[3], last_stride, bn_norm, with_se=with_se)    # [64, 2048, 14, 14]

        self.mixstyle = None
        if mixstyle_layers:
            self.mixstyle = []
            embed_dim = [256, 512, 1024, 2048]
            hw = [[56, 56], [28, 28], [14, 14], [14, 14]]
            for i in range(len(mixstyle_layers)):
                self.mixstyle.append(GlobalFilter(embed_dim[i], hw[i][0], hw[i][1]))
            print('Insert MixStyle after the following layers: {}'.format(mixstyle_layers))
            self.mixstyle = nn.Sequential(*self.mixstyle)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, bn_norm="BN", with_ibn=False, with_se=False, with_in=False):
        downsample = None
        ...
        layers = []
        layers.append(block(self.inplanes, planes, bn_norm, with_ibn, with_se, stride=stride, downsample=downsample))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks-1):
            layers.append(block(self.inplanes, planes, bn_norm, with_ibn, with_se))
        layers.append(block(self.inplanes, planes, bn_norm, with_ibn, with_se, with_in=with_in))
...
def build_resnet_fft_backbone(cfg):
    """
    ...
    with_in       = cfg.MODEL.BACKBONE.WITH_IN
    ...
    model = ResNet(last_stride, bn_norm, with_ibn, with_se, with_nl, with_in, mixstyle_layers, block,
